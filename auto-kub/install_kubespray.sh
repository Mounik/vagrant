...
echo "[kube_control_plane]">>inventory/mykub/inventory.ini
cat /etc/hosts | grep autokm | awk '{print $2}'>>inventory/mykub/inventory.ini

echo "[etcd]">>inventory/mykub/inventory.ini
cat /etc/hosts | grep autokm | awk '{print $2}'>>inventory/mykub/inventory.ini

echo "[kube_node]">>inventory/mykub/inventory.ini
cat /etc/hosts | grep autokn | awk '{print $2}'>>inventory/mykub/inventory.ini

echo "[calico_rr]">>inventory/mykub/inventory.ini
echo "[k8s_cluster:children]">>inventory/mykub/inventory.ini
echo "kube_control_plane">>inventory/mykub/inventory.ini
echo "kube_node">>inventory/mykub/inventory.ini
echo "calico_rr">>inventory/mykub/inventory.ini
