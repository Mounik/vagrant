...
cp /opt/janusgraph/conf/janusgraph-cql-es.properties /opt/janusgraph/conf/xavki.properties

sed -i s/"-Xms.*"/"-Xms2048M"/g /opt/janusgraph/conf/jvm-11.options
sed -i s/"-Xmx.*"/"-Xmx2048M"/g /opt/janusgraph/conf/jvm-11.options
sed -i s/"storage.hostname=.*"/"storage.hostname=janus1"/g /opt/janusgraph/conf/xavki.properties
#sed -i s/"index.search.backend=.*"/"index.search.backend=janusgraph"/g /opt/janusgraph/conf/xavki.properties
sed -i s/"index.search.hostname=.*"/"index.search.hostname=janus1"/g /opt/janusgraph/conf/xavki.properties
echo "graph.replace-instance-if-exists=true" >>/opt/janusgraph/conf/xavki.properties

sed -i s/"graph: conf\/janusgraph-inmemory.properties"/"graph: \/opt\/janusgraph\/conf\/xavki.properties"/g /opt/janusgraph/conf/gremlin-server/gremlin-server.yaml
